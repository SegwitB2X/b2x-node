...
var BitcoinRPC = require('b2x-rpc');
...
Bitcoin.prototype._initCaches = function() {
  ...
  this.countActiveAddressesCache = null;
  this.countActiveAddressesWithBalanceCache = null;
  this.averageBalanceCache = null;
};
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['countActiveAddresses', this, this.countActiveAddresses, 0],
    ['countAddressesWithBalance', this, this.countAddressesWithBalance, 0],
    ['averageBalance', this, this.averageBalance, 0],
  ];
  return methods;
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './bitcoin2x.conf');

  log.info('Using Segwit2x config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of b2x-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of b2x-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of b2x-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in bitcoin2x.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind(Segwit2x) is undergoing a reindex. ' +
             ...
             'of b2x-node services will start.');
    node._reindex = true;
  }
};
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Segwit2x Daemon Ready');
          callback();
        });
      });
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/bitcoin2x.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/bitcoin2x.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Segwit2x Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Segwit2x Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting Segwit2x process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Segwit2x process unexpectedly exited with code:', code);
        log.warn('Restarting Segwit2x child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Segwit2x process restarted');
          });
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Segwit2x configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @return {*}
 */
Bitcoin.prototype.countActiveAddresses = function(callback) {
    var self = this,
        countActiveAddressesCache = this.countActiveAddressesCache;

    if (countActiveAddressesCache !== null) {
        return setImmediate(function() {
            return callback(null, countActiveAddressesCache);
        });
    }

    return self._tryAllClients(function(client, done) {
        return client.countActiveAddresses(function(err, response) {

            if (err) {
                return done(self._wrapRPCError(err));
            }

            self.countActiveAddressesCache = response.result;
 ...
 * @return {*}
 */
Bitcoin.prototype.countAddressesWithBalance = function(callback) {

  var self = this,
      countActiveAddressesWithBalanceCache = this.countActiveAddressesWithBalanceCache;

    if (countActiveAddressesWithBalanceCache !== null) {
        return setImmediate(function() {
            return callback(null, countActiveAddressesWithBalanceCache);
        });
    }

    return self._tryAllClients(function(client, done) {
        return client.countAddressesWithBalance(function(err, response) {

            if (err) {
                return done(self._wrapRPCError(err));
            }

            self.countActiveAddressesWithBalanceCache = response.result;
 ...
 * @return {*}
 */
Bitcoin.prototype.averageBalance = function(callback) {

  var self = this,
      averageBalanceCache = this.averageBalanceCache;

    if (averageBalanceCache !== null) {
        return setImmediate(function() {
            return callback(null, averageBalanceCache);
        });
    }

    return self._tryAllClients(function(client, done) {
        return client.averageBalance(function(err, response) {

            if (err) {
                return done(self._wrapRPCError(err));
            }

            self.averageBalanceCache = response.result;
