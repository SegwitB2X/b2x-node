...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitcoind is necessary for this upgrade with the "reindex=1" bitcoin2x.conf option. \n' +
                 'There are changes necessary in both bitcoin2x.conf and b2x-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed b2x-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './b2x-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in b2x-node services directory
