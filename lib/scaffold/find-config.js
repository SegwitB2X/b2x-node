...
/**
 * Will return the path and b2x-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'b2x-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'b2x-node.json'))
  };
}
