...
var BASE_PACKAGE = {
  description: 'A full Segwit2x node build with Bitcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'b2x-core-lib': '^' + bitcore.version,
    'b2x-node': version
  }
};

/**
 * Will create a directory and bitcoin2x.conf file for Segwit2x.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/b2x-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the b2x-node directory and configuration
